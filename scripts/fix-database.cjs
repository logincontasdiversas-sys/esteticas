...
if (!supabaseUrl || !supabaseKey) {
  console.error('❌ Faltam credenciais no environment');
  process.exit(1);
}
...
async function fixDatabase() {
  ...
  try {
    ...
    if (orgError) {
      ...
      console.log('✅ Organização sincronizada com sucesso:', JSON.stringify(org));
    }
    ...
    if (profileError) {
      ...
      console.log('✅ Perfil vinculado com sucesso:', JSON.stringify(profile));
    }
